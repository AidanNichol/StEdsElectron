...
const atts = ['d', 'fill', 'stroke', 'stroke-linecap', 'stroke-linejoin',
...
export const parse = extract;

const cache = new Map();

export const drawSVG = (doc, x, y, scale, name, xtra = {})=>{
...
const drawPath = (doc, path, xtra)=>{
  ...
  if ((fill || path.fill) && (stroke || path.stroke)) return doc.fillAndStroke(fill || path.fill, stroke || path.stroke);
