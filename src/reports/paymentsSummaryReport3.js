...
export function paymentsSummaryReport3(payload, printFull){
  ...
  doc.on('pageAdded', ()=>{
    ...
    doc.fontSize(14).text(`${fmtDate(payload.startDate)} to ${fmtDate(payload.endDate)}`, 30, 30+(20+height14)/2, {align: 'center'})
  });
...
function prepareData(payload, printFull){
  ...
  payload.accounts
    ...
    .forEach(acc=>{
      ...
      accData.push({paymentsMade, accId, accName, memIds, addToCredit: available.P, grid})
    }
  )
  const walkIndex = new Map(Array.from(walkIds).sort((a,b)=>a>b).map((id, i)=>[id, {index: i}]));
  walkIndex.forEach((data, walkId)=>{
    data.name = WS.walks.get(walkId).names.code + ' ' + (new XDate(walkId.substr(1)).toString('dd MMM'));
  });
...
export function reportBody(payload, printFull, doc){
  ...
  const printColumnHeading = (x, y, walks )=>{
    ...
    const printCell = (i, text)=>{
      ...
      doc.font(normal).fontSize(szH).fillColor('black').text(text, x1, y+1, {align: 'center', width: bkWidth})
...
// routines for printing individual components
  ...
  const showMoney = (x, y, dY, value, text, rectFill, rectStroke, textColor)=>{
    ...
    doc.roundedRect(x1+boxOff,y+dY-1, boxWidth, detailH-2, r).fillAndStroke(rectFill, rectStroke)
  ...
  accData.forEach(acc=>{
    ...
    if (addToCredit){
      doc.fontSize(szD).text(`£${addToCredit}`, x+colW - bkWidth*(noWalks+1-0.5) - detailH*0.4, startY+dY, { height: detailH*.8})
    }
    if (paymentsMade > 0)showMoney(x, startY, dY, paymentsMade, 'Paid', "#cfe", "#484", 'blue');
    y = startY + accHeight;
    y += gapH;
  })
