...
export function paymentsSummaryReport(payload) {
  ...
  doc.on('pageAdded', () => {
    ...
    doc
      .fontSize(14)
      .text(`${payload.startDate} to ${payload.endDate}`, 30, 30 + (20 + height14) / 2, {
        align: 'center',
      });
  });
...
const reportBody = (payload, doc) => {
  ...
  let x,
    ...
    firstY,
    cX = margin + boxPad,
    dX = margin + colW + gutter,
    dW = 2 * colW - boxPad - gutter,
    cW = colW - boxPad - gutter;
  ...
  const AccLineTot = ({ y, x, wd, title, factor = '', item }) => {
    ...
    doc
      .fontSize(12)
      .fillColor('blue')
      .text(`${title} (${tots[item][0]})`, x + indent, y)
      .text(`${factor}£${tots[item][1]}`, x, y, {
        width: wd - 2 * indent,
        align: 'right',
      });
    return y + detailH;
  ...
  const AccLine = ({ y, x, wd, title, font = normal, factor = '', item, opt }) => {
    ...
    doc
      ...
      .fillColor('black')
      .text(title, x, y)
      .text(`${factor}£${item}`, x, y, { width: wd, align: 'right' });
  ...
  const printLogRec = (log, x, y) => {
    doc
      ...
      .image(`${__dirname}/../assets/icon-${log.req}.jpg`, x + 55, y - 3, {
        height: detailH * 0.9,
      })
      .fontSize(12)
      .text(`${log.text} `, x + 68, y, { continued: true })
      .font(italic)
      .fontSize(10)
  ...
  const printPayments = acc => {
    ...
    y += 3;
    let payments = acc.logs.filter(
      log => log.activeThisPeriod && log.paid && log.paid.P > 0,
    );
    let startY = y;
    ...
    doc
      .roundedRect(x - 2, y - 4, colW + 4, detailH + 4, 3)
