...
import {getSubsDue} from '../../../utilities/DateUtilities.js';
import {properCaseName, properCaseAddress, normalizePhone, normalize} from '../../utility/normalizers.js';
  ...
  render() {
    ...
    var title = (<h2>
      { firstName.value } { lastName.value } {changesMade ? '(changed)' : ''}
        <span style={{float: 'right', hidden:!(editMode && changesMade), cursor:'pointer'}} onClick={()=>setShowEditMemberModal(false)} >{showMode || changesMade?"" :"X"}</span>
    </h2>);
      ...
      <Panel bsStyle='info' className={"show-member-details "+(editMode ? 'editmode' : 'showMode')} header={title}  >
        ...
        <form className={'form-horizontal user-details modal-body ' + clss} name="user-details" autoComplete="off">
          ...
          <div className="form-line">
            ...
            <TextInput className="item-input" {...normalize(address, properCaseAddress)} />
          </div>
          <div className="form-line">
          ...
          <div className="form-line">
              ...
              <TextInput className="item-input" {...nextOfKin} />
          </div>
          <div className="form-line">
