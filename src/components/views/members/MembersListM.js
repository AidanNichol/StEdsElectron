...
@observer class Memberslist extends React.Component {

  render() {
    logit ('props', this.props);
    var {dispStart, dispLength, editMember, sortProp,
        editMode, allList, memberAdmin, newMember,
        setActiveMember, activeMemberId,

        setEditMode,  memberIndex,
        createNewMember,
        editFunctions,
        // setDeletePanding,
        // onChangeData,
        // resetEdit, saveEdit, deleteMember, cancelEdit,
        setDispStart, setSortProp,
} = this.props;
    ...
    var members = list.map((member)=>{
      ...
      let clss = classnames('list-line', [member.memberStatus]: true, {current: activeMemberId === member.memberId}, (member.suspended ? 'suspended' : subsStatus));
      // logit('classes', this.props.membersList.status)
      return (
        <div key={member._id} className={clss} onClick={()=>setActiveMember(member.memberId, dispStart)}>
    ...
    const pageDown = ()=>{setDispStart(Math.min(dispStart+dispLength, allList.length-0.5*dispLength))}
    const pageUp = ()=>{setDispStart(Math.max(dispStart-dispLength, 0))}
    ...
    return (
      <Panel header={title} className="member-list" id="steds_memberlist">
        ...
        <div className='sort-buttons' hidden={editMode}>
          <TooltipButton key="name"  className={sortProp === 'name' ? 'active' : ''} onClick={()=>setSortProp('name')}>sort by Name</TooltipButton>
          <TooltipButton key="number"  className={sortProp === 'id' ? 'active' : ''} onClick={()=>setSortProp('id')}>sort by Number</TooltipButton>
        </div>
        ...
        <EditMemberData className="details" {...{editMember, editMode, newMember, ...editFunctions, setEditMode, memberAdmin}}
          onSubmit={showResults} onRequestHide={()=>setEditMode(false)}
          style={{minHeight: '100%'}}/>
        ...
        <span className="action-buttons" hidden={editMode}>
          ...
          <PrintButton onClick={()=>membershipListReport(allList)} placement='right' tiptext={`Print Membership List (Sorted by ${sortProp})`} />
