...
var logit = Logit('color:yellow; background:blue;', 'showConflicts:view');

const ShowLogs = inject('MS')(observer(({memId, data, versions, MS})=>{
  logit('ShowLogs', {memId, data, versions})


return (
<div>
  <h5>{memId} {MS.members[memId].fullName}</h5>
  <table>
    <thead>
      <tr>
        <th>Date</th>
    ...
    <tbody>
      {
        Object.entries(data.logs).sort(logCmpDate).map(([dat, data])=>(<ShowLog key={dat} {...{dat,data}} />))
      }
    </tbody>
  </table>
</div>)
...
const ShowConflictsUnstyled = inject('WS')(observer(({WS, ...props})=> {
  const walk = WS.walks[WS.activeWalk||WS.conflictingWalks[0]._id]
    ...
    return (
      <Panel header={title} className={"showconflicts "+props.className}>
        <SelectWalk {...{setCurrentWalk: WS.setActiveWalk, walks: WS.conflictingWalks, currentWalk: WS.activeWalk||WS.conflictingWalks[0]._id}}/>
        <div>

          {  Object.entries(sum).map(([memId, data])=>( <ShowLogs key={memId} {...{memId, data, versions}}/>)) }
        </div>
