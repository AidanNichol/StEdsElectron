...
export const getSubsStatus = (doc)=>{
  ...
  let dueSubsYear = _today >= (new Date(`${thisYear}-11-31`)) ? thisYear+1 : thisYear;
  ...
  let showSubsButton = _today >= (new Date(`${thisYear}-11-01`)) ;
  if (currentUserSubs >= okSubsYear) fee = 13;

  if (currentUserSubs >= year || currentUserSubs >= dueSubsYear) return {due: false, status, year, fee, showSubsButton};
  status = 'due';
  if (currentUserSubs >= okSubsYear) fee = 13;
  else status = 'late';
