...
class AccountsStore {
  ...
  get unclearedBookings() {
    logit("unclearedBookings", this);
    let bookings = {};
    this.accounts.values().forEach(account => {
      // ["A1182", "A608"].forEach((accId)=>{
      let acc = account.accountStatus;
      for (let log of acc.logs.reverse()) {
        if (log.balance === 0) break;
        if (log.type !== "W") continue;
        logit("unclearedBookings", { log, bookings });
        bookings[log.accId] = [...(bookings[log.accId] || []), log];
      }
    });
    return bookings;
  }
