...
class MembersStore {
  ...
  @action syncToIndex(){
    ...
    logit('resync', {i, dispStart: this.dispStart, dispLength:this.dispLength, dispLast: this.dispStart + this.dispLength-1})
  ...
  @computed get membersSorted(){
    return (this.sortProp === 'name' ? this.membersSortedByName : this.membersSortedByMemNo);
  }

  @computed get membersSortedByName(){
    const cmp = (a, b) => coll.compare(a.fullNameR, b.fullNameR);
    return this.members.values().sort(cmp);

  }

  @computed get membersSortedByMemNo(){
  ...
  @action createNewMember = ()=>{
    const memNo = this.members.values().reduce((max, mem)=>Math.max(max, mem.memNo), 0)+1;
    this.editMember = new Member({_id: 'M'+memNo, memberId: 'M'+memNo, accountId: 'A'+memNo, newMember: true});
  ...
  @action setActiveMemberId = memberId=>{
    this.activeMemberId = memberId;
  }

  @action setSortProp = seq=>{
    this.sortProp = seq;
  }

  @action setDispStart = no=>{
    this.dispStart = no;
  }

  @action resetEdit = ()=>{
    if (!this.newMember){ // won't match if new member being created
  ...
  @computed get membersIndexByNumber(){
  ...
  @action loadMembers = async () => {
    ...
    runInAction('update state after fetching data', () => {
      ...
      if (getSettings('router.enabled') && savedValues){
        ...
        if (this.members.has(memId))this.activeMemberId = JSON.parse(savedValues).memberId;
      }
      this.loaded = true;
    })
  }
